...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleBilling(BillingException ex) {
        List<ErrorDetailResponse> details = ex.details().stream()
                .map(detail -> new ErrorDetailResponse(detail.field(), detail.issue(), detail.message()))
                .toList();
    ...
    public ResponseEntity<ErrorResponse> handleUnreadable(HttpMessageNotReadableException ex) {
        return badRequest(
                ...
                List.of(new ErrorDetailResponse("request", "INVALID_REQUEST", "Request body is invalid JSON.")));
    }
